...
exports.create = (req) => {
	return cabService.findOne({_id: req.body.cabId}).then(function(cab){
		if(cab && !cab.isAssigned) {
			return cabService.update({_id: cab._id}, {isAssigned: true}).then(function(){
...
exports.update = (req) => {
	...
	return hireService.findOne({_id: reqHire._id}).then(function(hire){
		if(hire && !hire.amount) {			
			return cabService.update({_id: hire.cabId}, {isAssigned: false, location: customerLocation}, {new: true}).then(function(cab){
				let hiredTime = new Date(reqHire.arrivedAt).getTime() - new Date(hire.createdAt).getTime();
				...
				return hireService.update({_id: hire._id}, {arrivedAt: new Date(reqHire.arrivedAt), amount: totalCharge}, {new: true});
			});			
			...
			throw new Xerror(`hire history with id ${req.body.hire._id} not found`);			
		}
	}).then(null, mongooseErrorHandler);		
};
